...
export const venue: VenueConfig = {
  ...
  slug: 'paramount',
  url: 'https://www.ohmyrockness.com/venues/brooklyn-paramoun',
}

// OMR wraps Ticketmaster links in ticketmaster.evyy.net with the real URL in ?u=
...
// Ticketmaster serves a generic category fallback when an artist has no photo.
...
export async function scrape(): Promise<string> {
  ...
  console.log(`Paramount (OMR): ${rawCards.length} rows on list page`)

  // Dedup by OMR slug — defends against the list showing a show twice
  // (featured + regular, pagination overlap, etc.)
  ...
  for (const card of toProcess) {
    ...
    try {
      ...
      // Open the event-details modal. Skip the price slider — TM price extraction
      // is flaky and slow; empty price renders as ¯\_(ツ)_/¯ in the UI.
      await detailPage
      ...
      const { image, description } = await detailPage.evaluate((fallbackPatterns) => {
        ...
        // Prefer og:image, fall back to first non-generic lineup photo.
        const og =
        ...
        // "Additional Info" has the full rich-text blurb with <br>s; fall back
        // to the Please Note section if it's missing.
