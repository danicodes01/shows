...
// OMR wraps third-party ticket URLs in ?u= redirects. DICE uses link.dice.fm
// short-links that redirect on navigation, so pass those through unchanged.
...
export async function scrape(): Promise<string> {
  ...
  const rawCards = await page.$$eval('.row.vevent', (rows) =>
    rows.map((row) => {
      ...
      const bands = Array.from(row.querySelectorAll('.bands.summary a'))
        .map((a) => a.textContent?.trim() ?? '')
        .filter(Boolean)
        .join(', ')
      // Union Pool uses `.stage` for show-series / benefit names (e.g. "NYIC Benefit")
      const stage = row.querySelector('.venue .stage')?.textContent?.trim() ?? ''
      const title = stage ? `${stage}: ${bands}` : bands
      const moreInfo =
  ...
  console.log(`Union Pool (OMR): ${rawCards.length} rows on list page`)
  ...
  for (const card of toProcess) {
    ...
    try {
      ...
      const { image, description, price, previewUrl, previewTrack } = await detailPage.evaluate(() => {
        // Event poster lives in the sidebar; the smaller venue thumbnail is
        // in a different component (EventDetailsVenue__Image) — don't grab that.
        const poster = document.querySelector(
          '[class*="EventDetailsImage__Container"] img',
        ) as HTMLImageElement | null
        const image = poster?.src ?? ''

        // About section renders markdown as a series of <p> tags
